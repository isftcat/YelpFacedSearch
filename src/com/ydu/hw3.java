...
public class hw3 {
    ...
    private JDatePicker memberSinceDatePicker;
    private JPanel queryShowPanel;
    private JFrame GUI;

    private ArrayList<JCheckBox> main_category_pool;
    private ArrayList<JCheckBox> sub_category_pool;
    private ArrayList<JCheckBox> attribute_pool;
    ...
    private String search_for_business;
    private String search_for_user;

    private static final int FRAME_WIDTH = 1200;
    private static final int FRAME_HEIGHT = 600;
    private static final String JDBC_DRIVER = "oracle.jdbc.driver.OracleDriver";
    private static final String ORACLE_URL = "jdbc:oracle:thin:@localhost:1521:ydu";
...
*/
        ...
        memberSinceDatePicker = new JDatePicker();
    ...
    public void initMainCategory()throws ClassNotFoundException, SQLException {
        main_category_pool = new ArrayList<>();
    ...
    public void updateSubCategory() throws SQLException, ClassNotFoundException {
        sub_category_pool = new ArrayList<>();
        ...
        if(!checked_main_categories.isEmpty()){
            ...
            while (resultSet.next()) {
                ...
                sub_category_pool.add(sub_category);
    ...
    public void getUserResults() throws SQLException, ClassNotFoundException{
        String query = "SELECT YU.name, YU.yelping_since, YU.average_stars, YU.user_id FROM YELP_USERS YU ";
        if((condition1.getSelectedIndex() > 0 && isNumber(textField4.getText()))||(condition2.getSelectedIndex() > 0 && isNumber(textField3.getText()))
                ...
                || memberSinceDatePicker.getModel().isSelected()){
            query = query + "WHERE ";
        }
        if(select2.getSelectedIndex() > 0){
            search_for_user = select2.getSelectedItem().toString();
        }else{
            search_for_user = "OR";
        }
        if(condition1.getSelectedIndex() > 0 && isNumber(textField4.getText())){
            query = query + "YU.review_count " + condition1.getSelectedItem().toString() + " " + textField4.getText() + " " + search_for_user + " ";
        }
        if(condition2.getSelectedIndex() > 0 && isNumber(textField3.getText())){
            query = query + "YU.friend_count " + condition2.getSelectedItem().toString() + " " + textField3.getText() + " " + search_for_user + " ";
        }
        if(condition3.getSelectedIndex() > 0 && isNumber(textField5.getText())){
            query = query + "YU.average_stars " + condition3.getSelectedItem().toString() + " " + textField5.getText() + " " + search_for_user + " ";
        }
        if(condition4.getSelectedIndex() > 0 && isNumber(textField2.getText())){
            query = query + "YU.votes " + condition4.getSelectedItem().toString() + " " + textField2.getText() + " " + search_for_user + " ";
        }
        if(memberSinceDatePicker.getModel().isSelected()){
            int month = memberSinceDatePicker.getModel().getMonth()+1;
            int year = memberSinceDatePicker.getModel().getYear();
            query = query + "YU.yelping_since" + " >= " + "TO_DATE('" + month + "-" + year + "','MM-YYYY') " + search_for_user + " ";
        }
        query = query.substring(0, query.length()-search_for_user.length()-2);
    ...
    public String getMainCategoryQuery(){
        ...
        for(String s: checked_main_categories){
            sub_query_mc = sub_query_mc + "(SELECT MC" + i + ".business_id FROM MAIN_CATEGORIES MC" + i + " WHERE MC" + i + ".main_category = '" + s + "') " + search_for_business + " ";
            i++;
        }
        if(sub_query_mc.length() > 0) sub_query_mc = sub_query_mc.substring(0, sub_query_mc.length()-search_for_business.length()-2);
        return sub_query_mc;
    }
    ...
    public String getSubCategoryQuery(){
        ...
        for(String s: checked_sub_categories){
            sub_query_sc = sub_query_sc + "(SELECT SC" + i + ".business_id FROM SUB_CATEGORIES SC" + i + " WHERE SC" + i + ".sub_category = '" + s + "') " + search_for_business + " ";
            i++;
        }
        if(sub_query_sc.length() > 0) sub_query_sc = sub_query_sc.substring(0, sub_query_sc.length()-search_for_business.length()-2);
        return sub_query_sc;
    }
    ...
    public String getAttributeQuery(){
        ...
        for(String s: checked_attributes){
            ...
            */
            key = key_value[0];
            value = key_value[1];
            sub_query_ba = sub_query_ba + "(SELECT BA" + i + ".business_id FROM BUSINESS_ATTRIBUTES BA" + i + " WHERE BA" + i + ".attribute_key = '" + key + "' AND BA" + i + ".attribute_value = '" + value + "') " + search_for_business + " ";
            i++;
        }
        if(sub_query_ba.length() > 0) sub_query_ba = sub_query_ba.substring(0, sub_query_ba.length()-search_for_business.length()-2);
        return sub_query_ba;
    }

    public String getReviewQuery(){
        String sub_query = "";
        String sub_query_yr = "SELECT YR.business_id, AVG(stars) AS average_stars, COUNT(votes) AS total_votes FROM YELP_REVIEWS YR ";
        if("AND".equals(select1.getSelectedItem().toString())){
            search_for_business = "AND";
        }else{
            search_for_business = "OR";
        }
        ...
        if((reviewStar.getSelectedIndex() > 0 && isNumber(reviewStarValue.getText())) || (reviewVotes.getSelectedIndex() > 0 && isNumber(reviewVotesValue.getText()))){
            ...
            if((reviewStar.getSelectedIndex() > 0 && isNumber(reviewStarValue.getText())) && (reviewVotes.getSelectedIndex() > 0 && isNumber(reviewVotesValue.getText()))){
                sub_query = sub_query + "YR1.average_stars " + reviewStar.getSelectedItem().toString() + " " + reviewStarValue.getText() + " " + search_for_business +" YR1.total_votes " + reviewVotes.getSelectedItem().toString() + " " + reviewVotesValue.getText();
    ...
    public void getBusinessResults() throws SQLException, ClassNotFoundException{
        String query = "SELECT YB.business_name, YB.city, YB.state, YB.stars, YB.business_id FROM YELP_BUSINESS YB WHERE YB.business_id IN(";
        String sub_query_mc = getMainCategoryQuery();
        String sub_query_sc = getSubCategoryQuery();
    ...
    public void show_user_review(String user_id) throws SQLException, ClassNotFoundException{
        String query = "SELECT review_id, name FROM YELP_REVIEWS, YELP_USERS WHERE author = '" + user_id + "' AND author = user_id";
        Connection connection = connect_to_db();
        Statement statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
        ResultSet resultSet = statement.executeQuery(query);
    ...
    public void show_business_review(String business_id) throws SQLException, ClassNotFoundException{
        String query = "SELECT review_id, name FROM YELP_REVIEWS, YELP_USERS WHERE business_id = '" + business_id + "' AND author = user_id";
        Connection connection = connect_to_db();
        Statement statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
        ResultSet resultSet = statement.executeQuery(query);
        String[] columnNames = {"Review_id", "User_name"};
